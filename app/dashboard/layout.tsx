      ...
      <header className="fixed top-0 left-0 right-0 h-24 bg-[#050507]/40 backdrop-blur-md border-b border-white/5 z-50 px-6 lg:px-12 transition-all">
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* User Profile */}
            <div tabIndex={0} role="button" onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') router.push('/dashboard/account') }} onClick={() => router.push('/dashboard/account')} className="flex items-center gap-3 bg-gradient-to-r from-blue-600/10 to-purple-600/10 pl-4 pr-1.5 py-1.5 rounded-xl border border-white/10 hover:border-blue-500/30 transition-all group cursor-pointer relative">
            ...
            >
              ...
              >
                <div className="flex items-center justify-between mb-10">
                  <h2 className="text-xl font-bold orbitron-font">Menu</h2>
                  <button aria-label="Close navigation menu" onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-white/5 rounded-lg">
                    <X aria-hidden="true" className="w-6 h-6" />
                  </button>
                </div>

                <nav aria-label="Mobile navigation" className="grid grid-cols-1 gap-3">
                {navigation.map((item) => (
                  <Link 
                    key={item.name} 
                    href={item.href}
